...
if len(sys.argv) > 1:
    # 传入是绝对路径
    if os.path.basename(sys.argv[1]) != sys.argv[1]:
...
for file_path in listdir:
    if os.path.isfile(file_path) and ((not startStr or file_path.startswith(startStr)) and (not endStr or file_path.endswith(endStr)) or file_path in sys.argv) and file_path != 'renameWinScreenshot.exe' and file_path != 'renameWinScreenshot.py':
        rename(file_path)
